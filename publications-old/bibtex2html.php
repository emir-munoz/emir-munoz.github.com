...
function bibtex2html($entry, $type, $accents, $hightlightName = '', $authorLimit = 0){
	...
	if(trim($ret)==""){
		...
		$publisherlinked = true;	// true if the publisherurl has been set in a link; it will be placed on the "in" part except if the url is there, in which case it will be placed on the publisher part
		...
		$ret .= ' <span class="webpdf"><a class="link" style="cursor: pointer;" data-toggle="collapse" data-target="#'.$key.'">[bib]</a></span>&nbsp;';
		...
		$ret .= '<div class="collapse" id="'.$key.'"><pre style="font-size:0.9em; color:red;">'.$bibsource."\r\n".'}</pre></div>';
	}
